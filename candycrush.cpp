...
void drawJewel(int x, int y, int jewelNum, bool combo4, bool combo5)
{
	switch (jewelNum)
	{
	case 0:
		myEllipse(x, y, x + 30, y + 30, 255, 0, 255);
		break;
	...
	case 2:
		myRectangle(x, y, x + 30, y + 30, 0, 0, 255, 0, 0, 255);
		break;

	case 3:
		myTriangle(x, y, x + 30, y + 30,x,y, 122, 30, 255);
		break;

	case 4:
		myRectangle(x, y, x + 30, y + 30, 220, 250, 30, 0, 0, 0);
		break;
	}
	if (combo5)
		myTriangle(x + 7, y + 7, x + 22, y + 22,x,y, 255, 255, 255);
	else if (combo4)
		myEllipse(x + 7, y + 7, x + 22, y + 22, 255, 255, 255);
...
//checks if candies match in horizontal or combos are being formed or blasted in the process
void checkHmatch(int arr[][cols], bool combo4[][cols], bool combo5[][cols], int& score)
{
	...
	for (int i = 0; i < rows; i++)
		for (int j = 0; j < cols - 2; j++)
			if (arr[i][j] == arr[i][j + 1] && arr[i][j] == arr[i][j + 2])
			{
				...
				if (combo5[i][j]) combo5Explosion(arr, combo4, combo5, j, i, score);
				else if (combo5[i][j + 1]) combo5Explosion(arr, combo4, combo5, j + 1, i, score);
				else if (combo5[i][j + 2]) combo5Explosion(arr, combo4, combo5, j + 2, i, score);
				else if (combo4[i][j]) combo4Explosion(arr, combo4, combo5, j, i, score);
				else if (combo4[i][j + 1]) combo4Explosion(arr, combo4, combo5, j + 1, i, score);
				else if (combo4[i][j + 2]) combo4Explosion(arr, combo4, combo5, j + 2, i, score);
...
//checks if candies match in vertical or combos are being formed or blasted in the process
void checkVmatch(int arr[][cols], bool combo4[][cols], bool combo5[][cols], int& score)
{
	...
	for (int i = 0; i < rows - 2; i++)
		for (int j = 0; j < cols; j++)
			if (arr[i][j] == arr[i + 1][j] && arr[i][j] == arr[i + 2][j])
			{
				...
				if (combo5[i][j]) combo5Explosion(arr, combo4, combo5, j, i, score);
				else if (combo5[i + 1][j]) combo5Explosion(arr, combo4, combo5, j, i + 1, score);
				else if (combo5[i + 2][j]) combo5Explosion(arr, combo4, combo5, j, i + 2, score);

				else if (combo4[i][j]) combo4Explosion(arr, combo4, combo5, j, i, score);
				else if (combo4[i + 1][j]) combo4Explosion(arr, combo4, combo5, j, i + 1, score);
				else if (combo4[i + 2][j]) combo4Explosion(arr, combo4, combo5, j, i + 2, score);
...
//print the score
void printScore(int score, DWORD& startTime)
{
	gotoxy(5, 1);
	cout << "Score: " << score;
	gotoxy(20, 1);
...
bool leftSwappable(int arr[][cols], int x, int y)
{
...
bool rightSwappable(int arr[][cols], int x, int y)
{
...
bool upSwappable(int arr[][cols], int x, int y)
{
...
bool downSwappable(int arr[][cols], int x, int y)
{
...
void keysHandler(int& x, int& y, bool& selected, int arr[][cols], bool combo4[][cols], bool combo5[][cols])
{
	int keyPressed = 0;
	if (isCursorKeyPressed(keyPressed))
	{
		...
		{
			if (keyPressed == 1 && x > 0 && leftSwappable(arr, x, y))
			{
				swap(arr[y][x], arr[y][x - 1]);
				swap(combo4[y][x], combo4[y][x - 1]);
				swap(combo5[y][x], combo5[y][x - 1]);
			}
			else if (keyPressed == 3 && x < cols - 1 && rightSwappable(arr, x, y))
			{
				swap(arr[y][x], arr[y][x + 1]);
				swap(combo4[y][x], combo4[y][x + 1]);
				swap(combo5[y][x], combo5[y][x + 1]);
			}
			else if (keyPressed == 2 && y > 0 && upSwappable(arr, x, y))
			{
				swap(arr[y][x], arr[y - 1][x]);
				swap(combo4[y][x], combo4[y - 1][x]);
				swap(combo5[y][x], combo5[y - 1][x]);
			}
			else if (keyPressed == 4 && y < rows - 1 && downSwappable(arr, x, y))
			{
				swap(arr[y][x], arr[y + 1][x]);
				swap(combo4[y][x], combo4[y + 1][x]);
				swap(combo5[y][x], combo5[y + 1][x]);
	...
	if (!selected)
		myRectangle(X1 + x * 50, Y1 + y * 50, X1 + 50 + (x * 50), Y1 + 50 + (y * 50), 250, 150, 130, 0, 0, 0);
	else
		myRectangle(X1 + x * 50, Y1 + y * 50, X1 + 50 + (x * 50), Y1 + 50 + (y * 50), 0, 255, 0, 0, 0, 0);
}

int main()
{
	int arr[rows][cols]; //the main 2d array which stores an int corresponding to the candy
	...
	randomJewels(arr);
	DWORD startTime = GetTickCount();
	DWORD endTime = GetTickCount() + (1000 * totalSeconds);

	while (GetTickCount() < endTime)
	{
		std::system("cls");
		myRectangle(X1 - 5, Y1 - 5, X1 + (50 * cols) + 5, Y1 + (50 * rows) + 5, 20, 230, 230, 0, 0, 0);
		printScore(score, startTime);
		keysHandler(activeX, activeY, selectedCell, arr, combo4, combo5);
		for (int i = 0; i < rows; i++)
		{
			for (int j = 0; j < cols; j++)
			{
				int candylocationX = X1 + (50 * j + 10);
				int candylocationY = Y1 + (50 * i + 10);
				drawJewel(candylocationX, candylocationY, arr[i][j], combo4[i][j], combo5[i][j]);
			}
		}
		checkHmatch(arr, combo4, combo5, score);
		checkVmatch(arr, combo4, combo5, score);
	}
	std::system("cls");
